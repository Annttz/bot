...
module.exports = async (client, inter) => {
    ...
    if (!command) return inter.editReply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription('❌ | Error! Please contact @myw ou @val')], ephemeral: true, }), client.slash.delete(inter.commandName)
    ...
    if (command.voiceChannel) {
            if (!inter.member.voice.channel) return inter.editReply({ embeds: [ new EmbedBuilder().setColor('#ff0000').setDescription(`❌ | Mets toi dans salon vocal bruh`)], ephemeral: true, })
